...
No model pin exists outside Claude Code — use the strongest available model if
the runtime lets you choose; correctness matters here. Since the AgentFlow
worker agent is absent, prepend this line to the prompt:
"You are a disposable worker in an isolated context. Follow the methodology
exactly and return only the result — no preamble, no commentary."
