...
### Added
- **Cross-runtime support (Codex and other AI coding tools).** Every `SKILL.md` now carries an "Other runtimes" section: same methodology prompt, dispatch via Codex `spawn_agent`/`wait_agent`/`close_agent` (needs `multi_agent = true` in `~/.codex/config.toml`) or any runtime's isolated-subagent mechanism, with a short worker preamble replacing the Claude-only `agents/*.md` definitions. Installer grows `--codex` (→ `~/.codex/skills/`) and `--dest <path>` (any skills dir, e.g. `~/.agents/skills/`) — skills only, no agents, since model pins don't exist outside Claude Code. Root `AGENTS.md` added for Codex contributors. Test suites extended (skills 62→83 checks, installer 11→20).
